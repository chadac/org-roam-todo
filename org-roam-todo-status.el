...
(defun org-roam-todo-status--insert-ci-section (feedback worktree-path)
  ...
  (let* ((ci-checks (plist-get feedback :ci-checks))
         ...
         (failed (or (plist-get summary :ci-failed-count) 0))
         (pending (or (plist-get summary :ci-pending-count) 0))
         (success (or (plist-get summary :ci-success-count) 0))
         (total (or (plist-get summary :ci-total-count) 0)))
